...
static void marker_bit (IBitStream &bs, unsigned int what)
{
    if (what != bs.Get1Bit())
    {
        mjpeg_error_exit1 ("Illegal MPEG stream at offset (bits) %lld: supposed marker bit not found.",bs.bitcount());
        // exit (1);
    }
}
...
void VideoStream::ScanFirstSeqHeader()
{
    ...
    {
		mjpeg_error_exit1 ("Invalid MPEG Video stream header.");
		// exit (1);
    }
...
void VideoStream::NextDTSPTS()
{
    ...
    else if( pulldown_32 )
	{
        present_fields = group_start_field  + startup_skew + decode_delay +
            gopfields_32pd( temporal_reference, repeat_first_field != 0 ); // CHANGED!
