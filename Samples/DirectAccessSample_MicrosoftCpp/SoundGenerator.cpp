// SoundGenerator.cpp: implementation of the CSoundGenerator class.
//
//////////////////////////////////////////////////////////////////////
...
unsigned int CSoundGenerator::Rnd(unsigned int *seed_ptr, int n)
{
...
int CSoundGenerator::IntCos(int a)
{
    int neg, v, f;
...
void CSoundGenerator::PutSample(int v)
{
	if (m_nBufPos >= m_nBufLen - 2)
		return;

	m_pBuffer[0] = v & 0xff;
	m_pBuffer[1] = (v >> 8) & 0xff;
...
HRESULT CSoundGenerator::GenerateSound(BYTE *pTargetBuffer, long nBufferLen)
{
	m_pBuffer = pTargetBuffer;
	m_nBufLen = nBufferLen;
    ...
    for(j=0;j<NB_CHANNELS;j++) {
        taba[j] = 0;
        tabf1[j] = 100 + Rnd(&seed, 5000);
        tabf2[j] = 100 + Rnd(&seed, 5000);
    }
    for(i=0;i<1 * FE;i++) {
        for(j=0;j<NB_CHANNELS;j++) {
            v = (IntCos(taba[j]) * 10000) >> FRAC_BITS;
            PutSample(v);
    ...
    for(i=0;i<2 * FE;i++) {
        for(j=0;j<NB_CHANNELS;j++) {
            amp = ((FRAC_ONE + IntCos(ampa)) * 5000) >> FRAC_BITS;
            if (j & 1)
                amp = 10000 - amp;
            v = (IntCos(a) * amp) >> FRAC_BITS;
            PutSample(v);
            a += (500 * FRAC_ONE) / FE;
            ampa += (2 * FRAC_ONE) / FE;
        }
